...
import '../main.dart'; // Akses liquidBackgroundNotifier global
...
class _LiquidGlassBackgroundState extends State<LiquidGlassBackground>
  ...
  void initState() {
    ...
    if (liquidBackgroundNotifier.value) {
      _controller.repeat();
    }

    liquidBackgroundNotifier.addListener(_handleSettingsChange);
  }
  ...
  @override
  void dispose() {
    liquidBackgroundNotifier.removeListener(_handleSettingsChange);
    _controller.dispose();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final baseBgColor = isDark ? const Color(0xFF090D19) : const Color(0xFFE8EEF5);
    ...
    return Scaffold(
      ...
      body: ValueListenableBuilder<bool>(
        ...
        builder: (context, enableLiquid, _) {
          if (!enableLiquid) {
            // Latar belakang statis performa tinggi tanpa animasi & blur fullscreen
            return SafeArea(
              bottom: false,
              child: widget.child,
            );
          ...
          // Latar belakang liquid glass asli dengan orbs bergerak & backdrop filter blur 90
          return Stack(
            children: [
              ...
              Positioned.fill(
                child: BackdropFilter(
                  ...
                  child: Container(
                    color: isDark ? Colors.black.withValues(alpha: 0.15) : Colors.white.withValues(alpha: 0.3),
                  ),
                ),
              ),
